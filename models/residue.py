...
class PerResidueEncoder(nn.Module):
    ...
    def __init__(self, feat_dim):
        super().__init__()
        self.aatype_embed = nn.Embedding(21, feat_dim)  # type of aa
        self.torsion_embed = PositionalEncoding()   # torsion=扭转
    ...
    def forward(self, aa, pos14, atom_mask):
        ...
        Args:
            aa:           (N, L).
            pos14:        (N, L, 14, 3). 14:[Ca, C, N, O]
            atom_mask:    (N, L, 14).  每条氨基酸序列哪些原子存在
        """
        N, L = aa.size()

        R = construct_3d_basis(pos14[:, :, 1], pos14[:, :, 2], pos14[:, :, 0])  # (N, L, 3, 3)
        t = pos14[:, :, 1]  # (N, L, 3) atom C
        crd14 = global_to_local(R, t, pos14)    # (N, L, 14, 3)
        crd14_mask = atom_mask[:, :, :, None].expand_as(crd14)
        crd14 = torch.where(crd14_mask, crd14, torch.zeros_like(crd14)) # (N, L, 14, 3)         
        ...
        place_mask = (aa_expand == rng_expand)  # 索引出对应的氨基酸类型的crd
        ...
        # Each residue was also assigned an initial feature encoding its amino acid type and the local geometry of its atoms
